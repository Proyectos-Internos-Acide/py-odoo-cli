...
- Precios fijos por combinación (por persona)
"""
...
PRODUCT_NAME = "Perú - Perú 10 días"
...
DESCRIPTION = (
    "Vive un viaje de 10 días por la costa, el desierto, los Andes y la Amazonía andina. "
    "Descubre maravillas como las Islas Ballestas, las enigmáticas Líneas de Nazca, el vibrante "
    "Cusco, el Valle Sagrado y comunidades locales. Este itinerario está diseñado para una conexión "
    "auténtica con la historia, la naturaleza y la cultura viva del Perú. La travesía culmina con el "
    "legendario Camino Inca de 4 días, en una experiencia transformadora y profundamente significativa. "
    "Disfruta de servicios confiables, guías expertos y logística de primer nivel. Perú te espera con "
    "mil emociones en un solo viaje."
)

# (lodging, room) -> price
PRICE_MATRIX = {
    ("HOTEL 3 ESTRELLAS", "SWD"): 4105.0,
    ("HOTEL 3 ESTRELLAS", "DWB"): 3765.0,
    ("BED & BREAKFAST", "SWD"): 3790.0,
    ("BED & BREAKFAST", "DWB"): 3628.0,
}
...
def upsert_variant_prices(client: OdooClient, tmpl_id: int, pricelist_id: int) -> None:
    ...
    if len(variant_price) < len(PRICE_MATRIX):
        raise RuntimeError("No se pudo mapear todas las variantes para Perú 10 días.")
...
def main() -> None:
    print("Configurando Perú - Perú 10 días...")
    client = OdooClient()
    uid = client.connect()
