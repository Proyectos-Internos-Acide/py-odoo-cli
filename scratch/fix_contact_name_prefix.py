...
print("1. Renombrando Contacto 136 a 'DIEGO acide'...")
models.execute_kw(DB, uid, PASS, 'res.partner', 'write', [[136], {'name': 'DIEGO acide'}])
print("✅ Contacto 136 renombrado.")

print("\n2. Actualizando la automatización para omitir 'Cliente:' al crear contactos...")
...
if not record.partner_id and record.email_from:
    ...
    if '@' in raw_email and len(raw_email) > 4:
        ...
        if partner:
            ...
            # Limpiar prefijos de etiquetas corchete como [INTERNO] o [PAGO]
            if contact_name.startswith('['):
                parts = contact_name.split(']', 1)
                if len(parts) > 1:
                    contact_name = parts[1].strip()

            # Limpiar prefijos como "Cliente:" o "Cliente :"
...
"""
    ...
    print("✅ Automatización actualizada con éxito.")
