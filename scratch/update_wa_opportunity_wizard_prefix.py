...
def main():
    print("==================================================================")
    print(" Updating WA Opportunity Wizard with [WhatsApp] - Prefix & Strict Chat ")
    print("==================================================================")
    ...
    wview_id = client.search_read('ir.ui.view', domain=[('name', '=', 'wtk.wa.create.opportunity.wizard.form')], fields=['id'])[0]['id']

    # -------------------------------------------------------------------------
    # 1. Server Action 696: Procesar Wizard & Crear Lead con [WhatsApp] -
    # -------------------------------------------------------------------------
...
if target_wiz:
    ...
    if existing_lead:
        ...
        # Caso B: Crear nueva Oportunidad con prefijo [WhatsApp] -
        stage_new = env['crm.stage'].search([('name', 'ilike', 'Nuevo')], limit=1)
        if not stage_new:
            stage_new = env['crm.stage'].search([], order='sequence asc', limit=1)

        lead_vals = {
            'name': op_name or ("[WhatsApp] - Cotización - " + str(partner.name if partner else raw_phone)),
...
"""
    ...
    client.write('ir.actions.server', [696], {
        'code': code_process_wizard,
        'model_id': model_id,
    })
    print("Updated Server Action ID 696")

    # -------------------------------------------------------------------------
    # 2. Server Action 697: Abrir Wizard Modal con [WhatsApp] - & Mensajes Estrictos
    # -------------------------------------------------------------------------
    code_open_wizard = """# Server Action 697: Abrir Wizard Modal con [WhatsApp] - & Mensajes Estrictos del Chat
channel = False
...
# Transcribir Chat WA estrictamente de ESTE canal
chat_html = ""
if channel:
...
# Pre-llenar nombre con prefijo [WhatsApp] -
op_name = "[WhatsApp] - Cotización - " + str(partner.name if partner else (channel.name if channel else raw_phone))
...
"""
    ...
    client.write('ir.actions.server', [697], {
        'code': code_open_wizard,
        'model_id': m_dc,
    })
    print("Updated Server Action ID 697")

    print("\n==================================================================")
    print(" UPDATE COMPLETE: [WhatsApp] - Prefix Added & Chat Extraction Strict ")
    print("==================================================================")
