...
def update_server_action():
    ...
    for wizard in records:
        ...
        if so:
            ...
            so.write({'x_custom_quote_wizard_id': wizard.id})
            
            report_obj = env.ref('sale.action_report_saleorder', raise_if_not_found=False)
            if report_obj:
                pdf_content, dummy = env['ir.actions.report'].sudo()._render_qweb_pdf(report_obj.id, [so.id])
                ...
                so.message_post(
                    body="Se ha generado y guardado la cotización personalizada (Cliente) con brochures incluidos.",
                    attachment_ids=[attachment.id]
                )

    report = env.ref('sale.action_report_saleorder', raise_if_not_found=False)
    if report and so_record:
        action = report.report_action([so_record.id])
    else:
        action = {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {'title': 'Cotización cliente', 'message': 'No se encontró el reporte estándar.', 'type': 'warning', 'sticky': False}
        }
"""
    client.write('ir.actions.server', [602], {'code': new_code})
    print("Server Action actualizada con puente estándar exitosamente!")
