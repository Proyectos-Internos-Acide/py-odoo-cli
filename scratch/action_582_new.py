try:
    ...
    if record.description and 'DETALLES DEL TOUR (FORMULARIO CUSTOM)' in record.description:
        ...
        # Enriquecer la nota interna con los datos de contacto si no están presentes
        contact_info_html = ""
        if client_name and 'Cliente:' not in raw_html and 'Nombre:' not in raw_html:
            contact_info_html += f"<br/>👤 <b>Cliente:</b> {client_name}"
        if record.email_from and 'Correo:' not in raw_html and 'Email:' not in raw_html:
            contact_info_html += f"<br/>📧 <b>Email:</b> {record.email_from}"
        if record.phone and 'Teléfono:' not in raw_html and 'Telefono:' not in raw_html and 'Phone:' not in raw_html:
            contact_info_html += f"<br/>📞 <b>Teléfono:</b> {record.phone}"

        if contact_info_html:
            # Insertar datos de contacto justo después del encabezado si es posible
            if '</strong>' in raw_html:
                parts = raw_html.split('</strong>', 1)
                html_desc = parts[0] + '</strong>' + contact_info_html + parts[1]
            else:
                html_desc = contact_info_html + "<br/>" + raw_html
        else:
            html_desc = raw_html
