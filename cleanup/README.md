...
📁 **Ubicación de esta carpeta:** `/home/acide/py-odoo-cli/cleanup/`
...
| Script | Descripción |
|--------|-------------|
| `cleanup_odoo.py` | ✅ **EL QUE FUNCIONA** — Limpieza completa vía JSON-RPC + SQL directo |
| `restore_action_643.py` | Restaura el código de la action 643 (correo de confirmación) |
...
```bash
cd /home/acide/py-odoo-cli/cleanup
```
...
```bash
../.venv/bin/python cleanup_odoo.py
```
...
```bash
../.venv/bin/python restore_action_643.py
```
...
**Solución:** Usar `cleanup_odoo.py` (ya usa JSON-RPC).
